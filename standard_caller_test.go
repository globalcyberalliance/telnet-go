...
func TestStandardCallerFromClientToServer(t *testing.T) {
	...
	for testNumber, test := range tests {
		...
		stdout := writeNopCloser(&stdoutBuffer)
		stderr := writeNopCloser(&stderrBuffer)

		var dataWriterBuffer bytes.Buffer
		dataWriter := newWriter(&dataWriterBuffer)
		dataReader := newReader(bytes.NewReader([]byte{})) // <----------------- The important difference between the 2 loops.
...
func TestStandardCallerFromServerToClient(t *testing.T) {
	...
	for testNumber, test := range tests {
		...
		var dataWriterBuffer bytes.Buffer
		dataWriter := newWriter(&dataWriterBuffer)
		dataReader := newReader(bytes.NewReader(test.Bytes))
